...
When the context variable is unset, or the manifest cannot be obtained (filesystem path missing or unreadable; S3 fetch fails with no cached copy), the transformer produces fully **stageable** target resources carrying placeholder code-asset and entry-point values (the same placeholders emitted in validation contexts) and records a warning diagnostic. The placeholders satisfy the target's concrete resource schemas, so validation, change staging, and dry-run plans all succeed without a manifest; deploying the placeholders fails at deploy time by design. This preserves the ability to lint and plan a blueprint before `celerity build` has run.
...
All entries below depend on section 3. The **user-namespace** entries (`celerity/config` stores) are stamped by the transformer for each resolved outbound handler link, with the concrete lambda -> store link keeping the store-identifier variable in sync at deploy time; the internal `resources` store's identifiers are set **directly by the transformer** as literals, not via a link (see the per-target contract for why). The table documents the final contract for the resource-link configuration store.
...
| `CELERITY_CONFIG_<NS>_STORE_ID` | 🔗 on link, per `celerity/config` | **transformer-stamped** reference to the store's derived identifier, resolved at deploy time; the concrete link's `envVarName` annotation makes the provider link maintain the same variable (see the per-target contract) | User namespaces declared via `celerity/config` resources. |
