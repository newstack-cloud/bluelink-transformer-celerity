...
| config-store access (§10) | `aws.lambda.ssm.<store>.envVarName = CELERITY_CONFIG_<NS>_STORE_ID`, `…accessLevel = read` (or the `aws.lambda.secretsmanager.<store>.*` equivalents for an all-secret store) — stamped on the **function**, keyed by the concrete store resource name `<store>` (§11) |
...
A missing or unavailable build manifest (and hence a missing `lambda.entryPoint`) is **not fatal**: per the build-manifest fallback contract ([index.md §1.4](index.md#14-celerity-cli-build-manifest)), the transformer emits the function with the same **placeholder** `code`/`Handler` values used in validation contexts (`placeholder-bucket`/`placeholder-key`, `placeholder.handler`) and attaches a per-handler warning diagnostic (`resources/handler/handler_aws_serverless_emit.go`, `loadCodeLocationInfo`). The placeholders satisfy the `aws/lambda/function` schema, so concrete validation and change staging succeed manifest-less; deploying them fails at deploy time by design. This preserves lint/plan before `celerity build` has run.
...
Handlers with identical link sets share one `aws/iam/role`, and the union of injected grants is exactly what each of them needs — so least privilege holds. Handlers touching different resources get different roles rather than silently inheriting each other's access. Role resource naming follows `celerityLambdaExec_<fingerprint>`; the DEPLOYED role name is app-scoped as `<appName>-lambdaExec-<fingerprint>` (app segment truncated to respect IAM's 64-char cap, placeholder app name in validation contexts) so two applications — or two concurrent test runs — with the same fingerprint never collide on CreateRole in a shared account.

**App-scoped deployed names are the general rule for every name the transformer invents**, not just roles. An author-provided `spec.name` is honoured verbatim (the author owns uniqueness, as with queues and buckets); every generated deployed identifier is `<appName>-<base>` via `shared.AppScopedPhysicalName`, with the app segment truncated to the tightest AWS limit among the identifiers derived from the base. Concretely: schedule rules (`<app>-<schedule>-rule`), queue→topic forwarders (`<app>-<queue>-fwd[-role]`), apigatewayv2 API names (`<app>-<api>-<protocol>`; the abstract API has no name field so these are always generated), and — when `spec.name` is omitted — the cache base (ElastiCache replication group ID, 40-char cap; its subnet group, AUTH-token secret and RBAC user/group ids derive from it) and the sqlDatabase base (RDS instance/cluster identifiers, 52-char cap so `-reader` fits 63 and `-proxy-role` fits IAM's 64; its subnet group, proxy and proxy role derive from it). Logical values stay app-agnostic: blueprint resource names, deploy-config lookup keys (`aws.elasticache.<name>.*`, `aws.aurora.<name>.*`), label values and the engine-internal `dbName`/`databaseName`.
...
**Handler wiring (labels + env vars).** The emitted concrete store carries the labels matched by inbound handler → config link edges — and only those. Labels matched by non-handler selectors (for example the api::config association, a no-op on `aws-serverless`) are deliberately not propagated: no concrete link implementation exists for them, and a label-matched candidate without a link implementation trips a blueprint-lib chain-construction bug (v0.51.0 `links/speclinkinfo.go`) that silently drops the store from change staging. With the matched labels in place, each linked handler's preserved `linkSelector` re-matches the concrete store and the §11 lambda → store link forms. On each linked handler's function the transformer stamps `CELERITY_CONFIG_<NS>_STORE_ID` — a reference to the store's derived `spec.id` value (path prefix or secret ARN), resolved at deploy time — and `CELERITY_CONFIG_<NS>_STORE_KIND` per §10.1. `<NS>` is the store name (`spec.name`, falling back to the blueprint resource name) upper-cased with every non-alphanumeric character mapped to `_` (`appConfig` → `APPCONFIG`).
...
| Backend | Link to declare | Link annotations (stamped on the **function**; `<store>` = the concrete store resource name, `<NS>` = the env-var namespace from §10.3) |
|---|---|---|
| `parameter-store` | `aws/lambda/function::aws/ssm/parameterTree` | `aws.lambda.ssm.<store>.accessLevel = read` (grants `ssm:GetParameter`, `ssm:GetParameters`, `ssm:GetParametersByPath` over the whole path), `aws.lambda.ssm.<store>.envVarName = CELERITY_CONFIG_<NS>_STORE_ID` |
| `secrets-manager` | `aws/lambda/function::aws/secretsmanager/secret` | `aws.lambda.secretsmanager.<store>.accessLevel = read` (grants `secretsmanager:GetSecretValue`, `secretsmanager:DescribeSecret`), `aws.lambda.secretsmanager.<store>.envVarName = CELERITY_CONFIG_<NS>_STORE_ID` |

`<store>` is the emitted store's blueprint resource name (`<config>_config_param_tree` or `<config>_config_secret`); the provider resolves annotation keys against the link target's resource name, so the annotation segment and the env-var namespace are distinct derivations.

The env var carries the store identifier. For Parameter Store that is the **path prefix** (the `parameterTree.path`); for Secrets Manager it is the secret **ARN** (`$.id` — this resource has no `arn` attribute). Either way **one link is one store**: the tree represents the whole namespace, so a handler declares a single link regardless of how many parameters sit beneath the path. The variable is assured twice over: the transformer stamps `CELERITY_CONFIG_<NS>_STORE_ID` on the function at transform time as a reference to the store's derived identifier (§10.3), and it also sets the link's `envVarName` annotation — whose default would otherwise be `SSM_PARAMETER_PATH_<name>` (the parameter-path link's convention) — so the deploy-time link injection maintains the same contract-mandated name (§5). `CELERITY_CONFIG_<NS>_STORE_KIND` remains a transformer-injected literal (§5), since no link supplies it.
