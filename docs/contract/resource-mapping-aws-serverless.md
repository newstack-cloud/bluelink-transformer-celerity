...
**Status:** implemented (emit + tests). The handler-side link is established the same way as every other backing target: the handler selects its config stores by label and the emitted Lambda carries the handler's `linkSelector` ([handler_aws_links.go](../../resources/handler/handler_aws_links.go)), so the `lambda/function::ssm/parameterTree` / `::secretsmanager/secret` provider links resolve by label (label-on-source) — no per-store `aws.lambda.ssm.*` / `aws.lambda.secretsmanager.*` annotation is emitted by the transformer. Specified in depth in [aws-serverless.md §10–§12](aws-serverless.md#10-runtime-configuration-store-aws-backends) and [index.md §1.3, §3.1](index.md#13-celerityconfig-abstract-resource).
