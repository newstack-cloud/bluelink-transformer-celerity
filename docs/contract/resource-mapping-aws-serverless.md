...
**Status:** implemented (`resources/api/api_aws_serverless_emit.go` + `api_transform_test.go`). Emits one `aws/apigatewayv2/api` **per protocol** + its `aws/apigatewayv2/stage`, JWT/REQUEST authorizers, and (when `spec.domain` is set) the custom-domain `domainName` + `apiMapping` resources; the `api::function` link owns the integration/route/permission plumbing. Each per-protocol API's `linkSelector` is the abstract API's selector **narrowed by a `celerity.internal.api.protocol` label** (`protocolScopedLinkSelector`, matched against `handler.LabelKeyAPIProtocol` on each handler), so a **hybrid** API's HTTP and WebSocket APIs each attach **only their own handlers** rather than sharing one selector (label-on-source); route/auth config is stamped on the **function** (resourceB), not the API.
...
| `externalEvents` dbStream/dataStream, or external SQS `sourceId` (URL/ARN) | **emitted standalone `aws/lambda/eventSourceMapping`** (`eventSourceArn` = the stream/queue ARN; URL sourceIds normalised to their ARN) | poll — no in-blueprint source, so the transformer owns the mapping |
| `externalEvents` objectStorage (out-of-blueprint bucket) | *(none — warning diagnostic)* | **deferred**, see §A — an S3 notification must be configured on the bucket, which is outside this blueprint |
...
**Annotation map:** handler outbound `aws.lambda.sqs.<queue>.accessLevel` (`send`\|`receive`\|`sendReceive`, default `send`), `.envVarName`, `.populateEnvVars`; consumer `aws.sqs.lambda.batchSize`, `aws.sqs.lambda.reportBatchItemFailures`; `deadLetterMaxAttempts` → **`aws.sqs.redrive.maxReceiveCount`** on the queue→queue link (→ `redrivePolicy.maxReceiveCount`). **Bucket notifications (implemented):** `celerity.queue.bucket.{events,filterPrefix,filterSuffix}` → `aws.s3.sqs.{event.<i>,filterPrefix,filterSuffix}` on the queue (`stampBucketNotifications`, consumed by the `s3/bucket::sqs/queue` link). `metadataUpdated` has no S3 equivalent → warning; when it is the **only** requested event, nothing is stamped and the receiver falls back to the provider default (`created` / `ObjectCreated`).
...
**Annotation map:** `aws.lambda.sns.<topic>.envVarName`, `.populateEnvVars` (no `accessLevel`). **Bucket notifications (implemented):** `celerity.topic.bucket.{events,filterPrefix,filterSuffix}` → `aws.s3.sns.{event.<i>,filterPrefix,filterSuffix}` on the topic (`stampBucketNotifications`, consumed by the `s3/bucket::sns/topic` link); `metadataUpdated` has no S3 equivalent → warning, and if it is the only requested event the receiver falls back to the provider default (`created` / `ObjectCreated`). Aside from the `celerity.topic.bucket.*` receiver annotations, topics have no `celerity.topic.*` annotations. **Do not** map any field to `contentBasedDeduplication` — the spec forbids it for FIFO topics; leave unset.
...
| `authMode="iam"` | `userGroupIds` → `aws/elasticache/user` + `aws/elasticache/userGroup` (IAM-enabled); handler stamps `aws.lambda.elasticache.<rg>.authMode=iam` |
...
**Status:** implemented (emit + tests, incl. env/link wiring). The handler-side link is established the same way as every other backing target: the handler selects its config stores by label, the emitted Lambda carries the handler's `linkSelector` ([handler_aws_links.go](../../resources/handler/handler_aws_links.go)), and the emitted concrete store carries the labels matched by inbound handler → config edges (only those — see [aws-serverless.md §10.3](aws-serverless.md#103-user-defined-celerityconfig-stores)), so the `lambda/function::ssm/parameterTree` / `::secretsmanager/secret` provider links resolve by label (label-on-source). The transformer stamps `CELERITY_CONFIG_<NS>_STORE_ID`/`_STORE_KIND` on each linked function and the per-store `aws.lambda.ssm.<store>.*` / `aws.lambda.secretsmanager.<store>.*` `envVarName`/`accessLevel` annotations on the **function**, keyed by the concrete store resource name. Specified in depth in [aws-serverless.md §10–§12](aws-serverless.md#10-runtime-configuration-store-aws-backends) and [index.md §1.3, §3.1](index.md#13-celerityconfig-abstract-resource).
...
|---|---|---|
| `queue → topic` (forward) | no `sqs::sns` link | **Resolved** (`resources/queue/queue_forwarder_aws_serverless.go`). Emit **one** intermediary `aws/lambda/function` per source queue (inline `code.zipFile` Node.js forwarder) + its base `aws/iam/role`. The source queue triggers the forwarder via a synthetic `celerity.queue.forward.<queue>` label activating the `sqs/queue::lambda/function` poll link; the forwarder's own `linkSelector` (union of the target topics' labels) activates a `lambda/function::sns/topic` link per topic, which injects each topic ARN under `CELERITY_FORWARD_TOPIC_ARN_<index>`. One forwarder fans out to **all** target topics (SQS is competing-consumers, so one poller per queue). See the v0 limitations below. |
...
**`queue → topic` forwarder — known v0 limitations.** The forwarder is implemented but constrained: (1) the forwarder's inline `code.zipFile` is **write-only** in the provider, so a change to the forwarder source only takes effect after the code object is refreshed (a source edit does not by itself force a redeploy); (2) **no KMS grants** are added for customer-managed-key-encrypted queues or topics, so a CMK-encrypted queue/topic will fail at runtime with `AccessDenied`; (3) a **FIFO queue ↔ FIFO topic** mismatch is not validated (MessageGroupId is passed through, but the shapes are not checked); (4) a queue that is **both consumed and forwarded** is an error diagnostic (a single SQS queue feeds one poller — the consumer's and the forwarder's would compete), and a forward edge carrying no target-topic label selector is likewise an error.
...
4. ~~**`vpc`**~~ — **Done** (emit). `aws/flex/vpc` with mode mapping, preset passthrough, deploy-config `cidrBlock`/DNS + `region`, reference-mode drops. Handler VPC *placement* (`vpcConfig` via `flex/vpc::function`, the `aws.flexvpc.lambda.subnetType` annotation) landed with the event-source/trigger work in step 6.
