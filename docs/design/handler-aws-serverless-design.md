...
The framework's `transformutils.RunTransformPipeline` already drives this loop (`bluelink: libs/plugin-framework/sdk/transformutils/pipeline.go`). Today it never runs because [../../transformer/transformer.go:81](../../transformer/transformer.go#L81) registers an empty `Aggregators` map; `Transform` therefore short-circuits to passthrough. Step 1 of "Cross-cutting wiring" below fixes that.
...
- **Emit** ([../../resources/handler/handler_aws_serverless_emit.go](../../resources/handler/handler_aws_serverless_emit.go)) — target-specific, per primary. Receives one `*ResolvedHandler`, builds the Lambda spec, and contributes inline-policy fragments to `SharedParentContributions["iam-role:<fp>"]` and layer-source fragments to `SharedParentContributions["layer:<hash>"]`. The framework's `mergeMappingNodesDeep` folds those into the seeded SharedParents from step (2).
...
emit(ctx, r, resPropRewriter, transformCtx) -> EmitResult
  ...
  spec := MappingNode{
    ...
    vpcConfig:       buildVPCConfig(r),                                  // omitted when r.VPC == nil
...
- Each handler emit then contributes via `SharedParentContributions[key]` to the parents this helper declared, and the framework's `mergeMappingNodesDeep` folds them. Byte-equal contributions for fingerprint-determined fields merge cleanly; the inline-policy union grows additively because each policy is keyed by name.

### [transformer/transformer.go](../../transformer/transformer.go) (modified)

Add `Aggregators: map[string]transformutils.Aggregator{shared.AWSServerless: aggregateAWSServerless(deps)}`. This is the one-line gap that prevents `RunTransformPipeline` from running today (line 81 is currently `Aggregators: map[string]transformutils.Aggregator{}` — empty map → `RunTransformPipeline` would error, so `TransformerPluginDefinition.Transform` short-circuits to passthrough at `bluelink: libs/plugin-framework/sdk/transformerv1/plugin_definition.go:148-153`).
...
1. **No aggregator registered**, so the pipeline never runs. [../../transformer/transformer.go:81](../../transformer/transformer.go#L81) is `Aggregators: map[string]transformutils.Aggregator{}`; `TransformerPluginDefinition.Transform` short-circuits to passthrough at `bluelink: libs/plugin-framework/sdk/transformerv1/plugin_definition.go:148-153`. One-line fix bundled with the aws-serverless aggregator above, but it is the gating change — no handler emit can run end-to-end until it lands.
...
2. **IAM role planner with SharedParent merge** is structurally non-trivial. Per-handler fragments must be byte-equal for fields owned by the fingerprint or `mergeMappingNodesDeep` returns a conflict, and the inline-policy set is the union across handlers sharing the role. Keep the seed-vs-contribution split tight (seed = base + assume-role; contribution = inline policies keyed by name) and expect to iterate on what counts as "fingerprint-determined" once link-derived policies start contributing real ARNs.
